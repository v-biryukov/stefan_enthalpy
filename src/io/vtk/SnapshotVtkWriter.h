...
class SnapshotVtkWriter: public BasicVtkWriter<typename ElasticSpace::SpaceType>
{
  public:
    ...
    void Write(const string& fileName, const string& dataName,
      Scalar* data,
      const Vector& origin, const Vector& spacing,
      const AABB& wholeBox, const AABB& box)
    {
      ...
      IntAABB iaabb;
      WriteHeaderOpenTags(file, origin, spacing, wholeBox, box, iaabb);

      IndexType snapshotSize = ::ComputeSnapshotSize<Space>(origin, spacing, box, &iaabb);

      std::string dataTypeString = "<PointData Scalars=\"" + dataName + "\">\n";
      file << dataTypeString;

      file << "<DataArray NumberOfComponents=\"1\" type=" << FloatTypeName() << " Name=\"" + dataName + "\" format=\"binary\">";
      WriteCompressedData(file, reinterpret_cast<unsigned char*>(data), snapshotSize * sizeof(Scalar));
      file << "</DataArray>\n";

      WriteHeaderCloseTags(file);
    }
  ...
  private:
    ...
    void WriteHeaderOpenTags(fstream& file,
      const Vector& origin, const Vector& spacing,
      const AABB& wholeBox, const AABB& box, IntAABB& iaabb) const
    {
    ...
    void WriteHeaderCloseTags(fstream& file) const
    {
      file << "</PointData>\n";
      file << "</Piece>\n";
      file << "</ImageData>\n";
