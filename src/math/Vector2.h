...
struct Vector2
{
  ...
  inline const Vector2<T> &operator *=(const T &i)			//mul number
  {
    x *= i;
    y *= i;
    return *this;
  }
  inline const Vector2<T> &operator /=(const T &i)			//div number
  {
    T inv = T(1.0) / i;
    x *= inv;
    y *= inv;
  ...
  inline const Vector2<T> &operator +=(const Vector2<T> &V)		//plus Vector3d
  {
    x += V.x;
    y += V.y;
    return *this;
  ...
  inline const Vector2<T> operator*(const T &d) const
  {
    return Vector2<T>(x * d, y * d);
  }
  const Vector2<T> GetPerpendicular() const
  {
